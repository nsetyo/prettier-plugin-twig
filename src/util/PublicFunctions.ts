...
export const firstValueInAncestorChain = (path, property, defaultValue?) => {
...
export const quoteChar = (options) => {
 ...
 */
const shouldExpressionsBeWrapped = (path): Symbol | boolean => {
	let result: Symbol | boolean = false
...
const wrapExpressionIfNeeded = (path, fragments, node: any = {}) => {
...
const isWhitespaceNode = (node: Node) => {
	return (
...
export const removeSurroundingWhitespace = (children: Node | Node[]) => {
	...
	return children.filter((child, index) => {
		...
		return !(isFirstOrLast && isWhitespaceNode(child))
	})
}
